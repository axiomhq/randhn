...
import { Story, StoryKind } from '../store/types';
...
class Home extends React.Component<NextPage, HomeState> {
  ...
  render() {
    const { currentStory, lastError, loading } = this.state;
    return (
  ...
  loadStory = async (storyType: StoryKind) => {
    this.setState({ lastError: undefined, loading: true });
    ...
    try {
      ...
      this.setState({ currentStory: story, loading: false });
    } catch (e) {
